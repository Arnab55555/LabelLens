...
def calculate_nutriscore():
    ...
    # Prepare data for Nutri-Score calculation
    nutriscore_data_ref = {
        'energy': safe_float(data.get('energy_kcal_100g')),
        'sugars': safe_float(data.get('sugars_100g')),
        'saturated_fat': safe_float(data.get('saturated_fat_100g')),
        'salt': safe_float(data.get('salt_100g')),
        'fiber': safe_float(data.get('fiber')),
        'fruits_vegetables_legumes': safe_float(data.get('fruits_vegetables_nuts_estimate_from_ingredients_100g')),
        'proteins': safe_float(data.get('proteins_100g')),
        'is_beverage': data.get('category', '').lower() == 'Beverages',
        'is_water': data.get('category', '').lower() == 'Water',
        'is_fat_oil_nuts_seeds': data.get('category', '').lower() == 'Fat/Nuts/Oil/Seeds',
        'is_red_meat_product': data.get('category', '') == 'Red Meat',
        'is_cheese': data.get('category','') in ('Cheese', 'Dairy')
}

    # Calculate Nutri-Score
    nutriscore_score = compute_nutriscore_score_2023(nutriscore_data_ref)
    nutriscore_grade = compute_nutriscore_grade_2023(
        nutriscore_score,
    ...
    # Return the result
    return jsonify({
        'nutriscore_score': round(nutriscore_score, 1),
...
#             else:
#                 # Check if the product contains any nutritional composition or is water
#                 if (row.get('category') == 'Water' or 
#                     any(row.get(nutrient) for nutrient in ['energy_100g', 'sugars_100g', 'saturated-fat_100g', 'salt_100g', 'fiber_100g', 'proteins_100g']) or 
#                     (safe_float(row.get('fruits-vegetables-nuts-estimate-from-ingredients_100g')) > 0)):
#                     nutriscore_data_ref = {
